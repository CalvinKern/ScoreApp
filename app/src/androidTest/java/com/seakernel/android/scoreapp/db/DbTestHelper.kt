...
class DbTestHelper {
    ...
    @Before
    fun createDb() {
        db = Room.inMemoryDatabaseBuilder(getAppContext(), AppDatabase::class.java).build()
    ...
    companion object {
        private const val DB_NAME = "${AppDatabase.DB_NAME}_test"
        ...
        private fun getAppDatabaseWithMigrations(): AppDatabase {
            return AppDatabase.getInstance(getAppContext(), DB_NAME)
        }

        fun createDatabaseAndMigrate(
            helper: MigrationTestHelper,
            startDbVersion: Int,
            endDbVersion: Int,
            migration: Migration,
            playerCount: Int = 2,
            block: (db: AppDatabase) -> Unit,
        ) {
            helper.createDatabase(DB_NAME, startDbVersion).apply {
            ...
            helper.runMigrationsAndValidate(DB_NAME, endDbVersion, true, migration)
        ...
        private fun insertScoresV4(score: Int, n: Int, db: SupportSQLiteDatabase) {
            ...
            db.execSQL(
                "INSERT INTO ${RoundEntity.TABLE_NAME} " +
            ...
            db.execSQL(
                "INSERT INTO ${ScoreEntity.TABLE_NAME} " +
        ...
        private fun insertGamePlayersV4(n: Int, db: SupportSQLiteDatabase) {
            ...
            db.execSQL(
                "INSERT INTO ${GamePlayerJoin.TABLE_NAME} " +
        ...
        private fun insertGamesV8(n: Int, db: SupportSQLiteDatabase) {
            val gameValues =
                List(n) { "($it, \"SimpleGame $it\", \"${now()}\", 0, 0, 0, 0, 0, 0, 0)" }
        ...
        private fun insertGamesV4(n: Int, db: SupportSQLiteDatabase) {
            ...
            db.execSQL(
                "INSERT INTO ${GameEntity.TABLE_NAME} " +
        ...
        private fun insertUsersRawV4(n: Int, db: SupportSQLiteDatabase) {
            ...
            db.execSQL(
                "INSERT INTO ${PlayerEntity.TABLE_NAME} " +
